<?php
// Get template args
extract(axiom_welldone_template_get_args('prev-next-block'));
...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = axiom_welldone_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo 'single-portfolio-fullscreen'==$page_style ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''; ?>>
			<?php 
			if ($page_style!='single-portfolio-fullscreen') axiom_welldone_show_layout($post_data['post_thumb']);
			$cur = get_post();
			$args = array(
			...
			$args = axiom_welldone_query_add_posts_and_cats($args, '', $post_data['post_type'], 
			...
			if ( $prev ) {
				...
				$desc = axiom_welldone_strshort($prev->post_title, 30);
				?>
			...
			if ( $next ) {
				...
				$desc = axiom_welldone_strshort($next->post_title, 30);
				?>
