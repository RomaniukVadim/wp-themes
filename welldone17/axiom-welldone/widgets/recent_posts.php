...
// Theme init
if (!function_exists('axiom_welldone_widget_recent_posts_theme_setup')) {
	add_action( 'axiom_welldone_action_before_init_theme', 'axiom_welldone_widget_recent_posts_theme_setup', 1 );
	function axiom_welldone_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('axiom_welldone_exists_visual_composer') && axiom_welldone_exists_visual_composer())
			add_action('axiom_welldone_action_shortcodes_list_vc','axiom_welldone_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('axiom_welldone_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'axiom_welldone_widget_recent_posts_load' );
	function axiom_welldone_widget_recent_posts_load() {
		register_widget('axiom_welldone_widget_recent_posts');
	}
}

// Widget Class
class axiom_welldone_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'axiom-welldone'));
		parent::__construct( 'axiom_welldone_widget_recent_posts', esc_html__('Welldone - Recent Posts', 'axiom-welldone'), $widget_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'axiom-welldone')), axiom_welldone_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'axiom_welldone_sc_widget_recent_posts' ) ) {
	function axiom_welldone_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'axiom_welldone_widget_recent_posts';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, axiom_welldone_prepare_widgets_args(axiom_welldone_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('axiom_welldone_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	axiom_welldone_require_shortcode("trx_widget_recent_posts", "axiom_welldone_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('axiom_welldone_widget_recent_posts_reg_shortcodes_vc')) {
	function axiom_welldone_widget_recent_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Posts", 'axiom-welldone'),
				"description" => wp_kses_data( __("Insert recent posts list with thumbs and post's meta", 'axiom-welldone') ),
