...
function axiom_welldone_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = AXIOM_WELLDONE_STORAGE['co_add_params']!=undefined ? AXIOM_WELLDONE_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#axiom_welldone-customizer-styles-'+option).length > 0 ? jQuery('#axiom_welldone-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="axiom_welldone-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
