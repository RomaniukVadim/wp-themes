						...
						<div class="container">
							<div class="row">
								...
								<div class="col-lg-10">
									...
									echo '<span class="quantities">' . $cart_item['quantity'] . ' &times; ' . wc_price( $_product->get_price() ) . '</span>';
									?>
								</div>
							</div>
						</div>
...
function adjacent_post_link_product( $format, $link, $in_same_cat = false, $excluded_categories = '', $previous = true ) {
	...
	} else { $post = get_adjacent_post_product( $in_same_cat, $excluded_categories, $previous );
	}
...
function get_adjacent_post_product( $in_same_cat = false, $excluded_categories = '', $previous = true ) {
	...
	if ( $in_same_cat || ! empty( $excluded_categories ) ) {
		...
		if ( $in_same_cat ) {
			...
			$cat_array = wp_get_object_terms( $post->ID, 'product_cat', array(
				'fields' => 'ids',
			) );
...
if ( ! function_exists( 'cg_woocommerce_upsell_display' ) ) {
	 ...
	 */
	function cg_woocommerce_upsell_display( $limit = '-1', $columns = 4, $orderby = 'rand', $order = 'desc' ) {
		...
		wc_get_template( 'single-product/up-sells-bottom.php', array(
			'upsells' => $upsells,
